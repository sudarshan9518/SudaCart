...
const Cart = () => {
    ...
    const [selectedAddres, setSelectedAdrdress]= useState(null)
        ...
        try{
            ...
            if(data.success){
                ...
                if(data.addresses.length >0){
                    setSelectedAdrdress(data.addresses[0])
                }
            }else{
        ...
        <div className="flex flex-col md:flex-row mt-16">
            <div className='flex-1 max-w-4xl'>
                ...
                <div className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 text-base font-medium pb-3">
                    ...
                    <p className="text-center">Subtotal{currency}</p>
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        ...
                        <button onClick={()=>removeFromCart(product._id)} className="cursor-pointer mx-auto">
                           <img src={assets.remove_icon} alt="remove_icons"  className="inline-block w-6 h-6"/>
                        </button>
                    </div>)
                )}
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                ...
                <div className="mb-6">
                    ...
                    <div className="relative flex justify-between items-start mt-2">
                        <p className="text-gray-500">{selectedAddres ?`${selectedAddres.street}, ${selectedAddres.city},${selectedAddres.state},${selectedAddres.country}` :"No address found"}  </p>
                        ...
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                              {addresses.slice(0,5).map((address, index)=>(
                                 <p key={index} onClick={() =>{ setSelectedAdrdress(address); setShowAddress(false)}} className="text-gray-500 p-2 hover:bg-gray-100">
                ...
                <button onClick={placeOrder} className="w-full py-3 mt-6 cursor-pointer bg-primary text-white font-medium hover:bg-primary-dull transition">
                   {paymentOption ==="COD" ?"Place Order" : "Proceed to Cheack Out"}
                </button>
            </div>
        </div>
    ) :(null)
